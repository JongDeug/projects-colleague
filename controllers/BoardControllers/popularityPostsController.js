...
const getMethod = async (req, res, next) => {
    ...
    try {
        ...
        for(const key in DB.Post){
            ...
            result[key] = await DB.Post[key].aggregate().addFields({"likeHitLength":{"$size" : "$likeHit"}}).match({"likeHitLength" : {"$gte" : variableDB.likeVar}}).sort({"likeHitLength": -1}).limit(variableDB.likePage);
        }
